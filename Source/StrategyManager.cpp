...
void StrategyManager::addStrategies() 
{
	...
	protossOpeningBook[fastZealotRush] = "0 0 0 0 1 0 3 3 0 0 4 1 4 4 0 4 4 0 1 4 3 0 1 0 4 0 4 4 4 4 1 0 4 4 4";
	...
	if (selfRace == BWAPI::Races::Protoss)
	{
		...
		{
			usableStrategies.push_back(fastZealotRush);
...
void StrategyManager::readResults()
{
	...
	else
	{
		...
		std::string line;
		getline(f_in, line);
		results[fastZealotRush].first = atoi(line.c_str());
		getline(f_in, line);
		results[fastZealotRush].second = atoi(line.c_str());
		getline(f_in, line);
...
void StrategyManager::writeResults()
{
	...
	//move fast rush depending on where it is in enum:
	f_out << results[fastZealotRush].first   << "\n";
	f_out << results[fastZealotRush].second  << "\n";
...
void StrategyManager::setStrategy()
{
	...
	else
	{
        ...
        else
        {
			...
			currentStrategy = fastZealotRush;
        }
	}
...
const MetaPairVector StrategyManager::getBuildOrderGoal()
{
	if (BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Protoss)
	{
		...
		else if (getCurrentStrategy() == fastZealotRush)
		{
			return getFastZealotRushBuildOrderGoal();
		}
...
const MetaPairVector StrategyManager::getFastZealotRushBuildOrderGoal() const
{
	// the goal to return
	MetaPairVector goal;
